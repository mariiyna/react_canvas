...
import {
  ...
  MAX_WIDTH,
  MIN_WIDTH,
} from '@/features/draw/model/CONSTS.ts';
import React, { useEffect, useState } from 'react';
import { ManagePanel } from '@/features/manageDraw/ui/ManagePanel.tsx';
import {
  redoStep,
  undoStep,
} from '@/features/manageDraw/model/historySlice.ts';
...
export function CanvasBoard() {
  ...
  const { history, currentIndex } = useSelector(
    (state: RootState) => state.history
  );
  const dispatch = useDispatch();

  const { canvasRef, getCoordinates, clearCanvas } = useCanvas(toolWidth);
  const { startDrawing, drawMove, stopDrawing } = useDrawing(
    canvasRef,
    activeTool,
    getCoordinates,
    toolColor
  ...
  }, [currentIndex, canvasRef, history]);
